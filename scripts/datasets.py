# From https://github.com/harsha-simhadri/big-ann-benchmarks
import gzip
import shutil
import math
import numpy
...
class RandomDSEasy(DatasetCompetitionFormat):
    def __init__(self, nb, nq, d, basedir="randomeasy"):
        self.nb = nb
        self.nq = nq
        self.d = d
        self.dtype = 'float32'
    ...
    def prepare(self, skip_data=False):
        ...
        nbrs = NearestNeighbors(n_neighbors=10, metric="euclidean", algorithm='brute').fit(data)
        D, I = nbrs.kneighbors(queries)
        with open(os.path.join(self.basedir, self.gt_fn), "wb") as f:
            np.array([self.nq, 10], dtype='uint32').tofile(f)
            I.astype('uint32').tofile(f)
            D.astype('float32').tofile(f)
...
DATASETS = {
    ...
    'random-easy-xs': lambda : RandomDSEasy(10000, 1000, 20),
    'random-easy-s': lambda : RandomDSEasy(100000, 1000, 50),
    'random-easy-s2': lambda : RandomDSEasy(100001, 1000, 200),
    'random-easy': lambda : RandomDSEasy(1000000, 1000, 200),
