...
public:
  ...
  void prefetch_neighbors(vidType v) const {
    __builtin_prefetch(edges+vertices[v]);
  }
...
#include <immintrin.h>
class RegularGraph: public Graph {
  public:
    using Graph::Graph;
    void fastprefetch(vidType v, _mm_hint hint = _MM_HINT_T0) const {
#ifdef USE_PF_GRAPH
      int l = (max_degree+15)/16;
      vidType *start = edges+(eidType) max_degree*v;
      for (int i = 0; i < l; i++) {
        __builtin_prefetch(start+16*i);
        //_mm_prefetch(start+16*i, hint);
      }
#endif
    }

    VertexSet fastN(vidType v) {
      return VertexSet(edges+(eidType) max_degree*v,max_degree,v);
    }

    void load_parlay(string file_name) {
      ...
      reader.read((char*)(&max_deg), 4);
      max_degree = max_deg;

      vector<int> degrees(num_points);
      reader.read((char*)degrees.data(), 4*num_points);
      edges = new vidType[num_points*max_deg];
      memset(edges,0,sizeof(vidType)*num_points*max_deg);

      /*int sum = 0;
      for (int x: degrees) sum += x;
      ofstream meta(file_name+".meta");
      meta << sum << " edges" << endl;
      for (int x: degrees) meta << x << " ";
      meta << endl;*/

      for (int i = 0; i < num_points; i++) {
        reader.read((char*)&edges[i*max_deg],4*degrees[i]);
      }
      reader.close();
    }
};
