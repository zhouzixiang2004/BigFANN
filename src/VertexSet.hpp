...
public:
  ...
  vidType intersect_buf(vidType *outBuf, const VertexSet &other) const {
    vidType idx_l = 0, idx_r = 0;
    ...
    while(idx_l < set_size && idx_r < other.set_size) {
      ...
      if(left <= right) idx_l++;
      if(right <= left) idx_r++;
      if(left == right) outBuf[return_set_size++] = left;
    }
  ...
  bool subsetof(const VertexSet &other) const {
    vidType idx_r = 0;
    for (vidType idx_l = 0; idx_l < set_size; idx_l++) {
      vidType left = ptr[idx_l];
      while (idx_r < other.set_size && other.ptr[idx_r] < left)
        idx_r++;
      if (idx_r == other.set_size || other.ptr[idx_r] > left) return false;
    }
    return true;
  }

  bool contains(const vidType &v) const {
    if (set_size > 32) {
      int l = 0,r = set_size-1;
      while (l <= r) {
        int m = (l+r) / 2;
        if (ptr[m] == v) return true;
        else if (ptr[m] < v) l = m+1;
        else r = m-1;
      }
      return false;
    }
